...
nz = round(diff_data_node_pos(3)/mesh_size*20 + 200);
dx = mesh_size;
dy = mesh_size;
dz = mesh_size;
...
% update center
z_loc = min(model1.nodePos(3, :)) + 25*dz;
...
chunk_sizes = [8e-4 7e-4 6e-4] * 1e6; % without abs boundary
...
for i = 1:numel(chunk_sizes)
    ...
    % cut the model
    elsDelete = find(X<int32(cx*1e6)-chunk_size | X>int32(cx*1e6)+chunk_size ...
        | Y<int32(cy*1e6)-chunk_size | Y>int32(cy*1e6)+chunk_size);
    ...
    xLims    = [ ];
    ylims    = [ ];
    zLims    = [z_min z_min+abs_size z_max-abs_size z_max];
