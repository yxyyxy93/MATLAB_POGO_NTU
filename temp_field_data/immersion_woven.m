...
% for idx = 2:numel(fields_45n45)
%     fieldName  = fields_45n45{idx};
%     fieldValue = data_45n45.elsets.(fieldName);
...
%     data_45n45.elsets.(fieldName) = newSet;
% end
% 
...
nz = round(diff_data_node_pos(3)/mesh_size*20*1.8);
dx = mesh_size;
dy = mesh_size;
dz = mesh_size;
...
%     model1.elNodes.', find(model1.matTypeRefs==6).', 0.2);
...
z_max_model = min(model1.nodePos(3, :));
z_loc       = z_max_model;
...
save([currentFileFolder '/base_model_20l'], 'model1', 'timestep', 'tb_signal', 'dz', 'center', '-v7.3');
