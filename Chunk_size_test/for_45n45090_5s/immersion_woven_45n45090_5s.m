...
nz = round(diff_data_node_pos(3)/mesh_size*12 + 190);
dx = mesh_size;
dy = mesh_size;
dz = mesh_size;
...
for idx = 1: size(focused_waves, 1)
    model1.shots{1, 1}.sigs{idx, 1}.nodeSpec   = node_index_generator(idx)';
    model1.shots{1, 1}.sigs{idx, 1}.sig        = focused_waves(idx, :)';
    model1.shots{1, 1}.sigs{idx, 1}.sigType    = 1; % 0 - force, 1 - displacement
    model1.shots{1, 1}.sigs{idx, 1}.isDofGroup = 0;
    model1.shots{1, 1}.sigs{idx, 1}.dofSpec    = 3;
    model1.shots{1, 1}.sigs{idx, 1}.sigAmps    = 1e-13;
end
...
chunk_sizes = [20e-4 10e-4 8e-4 6e-4 4e-4] * 1e6; % without abs boundary
clc;
...
for i = 1:numel(chunk_sizes)
    ...
    for idx = 1: size(focused_waves, 1)
        mDel.shots{1, 1}.sigs{idx, 1}.nodeSpec   = node_index_generator(idx)';
        mDel.shots{1, 1}.sigs{idx, 1}.sig        = focused_waves(idx, :)';
        mDel.shots{1, 1}.sigs{idx, 1}.sigType    = 1; % 0 - force, 1 - displacement
        mDel.shots{1, 1}.sigs{idx, 1}.isDofGroup = 0;
        mDel.shots{1, 1}.sigs{idx, 1}.dofSpec    = 3;
        mDel.shots{1, 1}.sigs{idx, 1}.sigAmps    = 1e-13;
    end
    % Receiver
    % node_index_receiver = node_index_generator;
    center(3) = z_loc + dz;
